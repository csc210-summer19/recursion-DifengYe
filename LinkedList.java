...
public class LinkedList<E extends Comparable<E>> {
	...
	public void removeAll(E el) {
		...
		removeAll(first, first.next, el);
	}

	public void removeAll(Node ref, Node refNext, E el) {
		if (refNext == null) {
			return;
		} else if (ref == first && first.data.equals(el)) {
			first = first.next;
			n--;
			removeAll(first, first.next, el);

		} else {
			if (refNext.data.equals(el)) {
				ref.next = refNext.next;
				n--;
				removeAll(ref, refNext.next, el);
			} else {
				removeAll(ref.next, refNext.next, el);
			}
		}
	}
